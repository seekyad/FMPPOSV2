  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
          ...
          <tbody>
            {rows.map((s) => (
              <tr key={s.id}>
                ...
                <td style={{ padding: '10px 16px', borderBottom: '1px solid var(--line-soft)', textTransform: 'capitalize' }}>{s.role}</td>
                <td style={{ padding: '10px 16px', borderBottom: '1px solid var(--line-soft)' }}>{s.isTechnician ? 'Yes' : '—'}</td>
                ...
                <td style={{ padding: '10px 16px', borderBottom: '1px solid var(--line-soft)', textAlign: 'right' }}>
                  {isManager && (
                    <span style={{ display: 'inline-flex', gap: 10 }}>
                      <button onClick={() => { setPinReset(s); setNewPin(''); }} style={{ border: 'none', background: 'none', color: 'var(--ink-3)', fontSize: 12 }}>
                        Reset PIN
                      </button>
                      <button
                        onClick={() => void patch(s.id, { role: s.role === 'manager' ? 'employee' : 'manager' })}
                        style={{ border: 'none', background: 'none', color: 'var(--ink-3)', fontSize: 12 }}
                      >
                        Make {s.role === 'manager' ? 'employee' : 'manager'}
                      </button>
                      <button
                        onClick={() => void patch(s.id, { active: !s.active })}
                        style={{ border: 'none', background: 'none', color: s.active ? 'var(--red)' : 'var(--green)', fontSize: 12 }}
                      >
      ...
      <Modal open={adding} onClose={() => setAdding(false)} width={380}>
        <h2 style={{ margin: 0, font: '700 17px Inter, sans-serif' }}>Add staff</h2>
        <input
          value={form.name}
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
          ...
          <select
            ...
            style={{ padding: '10px 12px', borderRadius: 10, border: '1px solid var(--line)', fontSize: 13, background: 'var(--card)' }}
          >
            <option value="employee">Employee</option>
            <option value="manager">Manager</option>
          </select>
        </div>
        <label style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 10, fontSize: 12, color: 'var(--ink-2)' }}>
      ...
      <Modal open={pinReset !== null} onClose={() => setPinReset(null)} width={320}>
        <h2 style={{ margin: 0, font: '700 17px Inter, sans-serif' }}>New PIN for {pinReset?.name}</h2>
        <input
          value={newPin}
