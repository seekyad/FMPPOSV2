  ...
  return (
    <div style={{ padding: '22px 24px', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ margin: 0, font: '700 24px Inter, sans-serif' }}>Activations</h1>
      ...
      <div style={{ marginTop: 14, background: 'var(--card)', borderRadius: 14, border: '1px solid var(--line-soft)', overflow: 'auto', flex: 1 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id}>
                <td style={{ padding: '11px 16px', borderBottom: '1px solid var(--line-soft)', font: '600 13px Inter, sans-serif' }}>{r.customerName}</td>
                <td style={{ padding: '11px 16px', borderBottom: '1px solid var(--line-soft)' }}>{KIND_LABELS[r.kind] ?? r.kind}</td>
                <td style={{ padding: '11px 16px', borderBottom: '1px solid var(--line-soft)', color: 'var(--ink-2)' }}>
                  {r.carrier}{r.planName ? ` · ${r.planName}` : ''}
                </td>
                <td style={{ padding: '11px 16px', borderBottom: '1px solid var(--line-soft)', color: 'var(--ink-3)' }}>{r.accountNumber ?? '—'}</td>
                <td style={{ padding: '11px 16px', borderBottom: '1px solid var(--line-soft)', color: 'var(--ink-3)' }}>{r.phoneNumber ?? '—'}</td>
                <td style={{ padding: '11px 16px', borderBottom: '1px solid var(--line-soft)' }}>{r.monthlyCents > 0 ? `${formatCents(r.monthlyCents)}/mo` : '—'}</td>
        ...
        {rows.length === 0 && <div style={{ padding: 24, color: 'var(--ink-4)', fontSize: 13 }}>No activations yet.</div>}
      </div>

      <Modal open={creating} onClose={() => setCreating(false)} width={460}>
        <h2 style={{ margin: 0, font: '700 18px Inter, sans-serif' }}>New activation</h2>
        <button
          onClick={() => setPickingCustomer(true)}
          style={{ width: '100%', marginTop: 12, padding: '10px 12px', borderRadius: 10, border: `1px solid ${customer ? 'var(--line)' : 'var(--orange)'}`, background: customer ? 'var(--card)' : 'var(--orange-soft)', textAlign: 'left', fontSize: 13 }}
        >
          {customer ? (
        ...
        <textarea
          ...
          style={{ width: '100%', marginTop: 8, padding: '10px 12px', borderRadius: 10, border: '1px solid var(--line)', fontSize: 13, resize: 'none' }}
        />
